...
class TableRotationSolver:
    ...
    def __init__(
        ...
        if n_students % n_tables != 0:
            raise ValueError("Number of students must be divisible by number of tables")
    ...
    def _handle_odd_student(
        ...
        missing_student = tuple(set(range(self.n_students)) - set(itertools.chain.from_iterable(tables)))
        
        # Find best table to add the odd student to
        table_scores = [
            sum(len(set(table + missing_student).intersection(prev_table))
                for prev_table in used_tables)
            for table in tables
        ]
